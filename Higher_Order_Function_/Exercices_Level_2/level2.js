...
function letterNumber() {
  ...
  for (let i = 0; i < (alphaChar.length); i++) {
    ...
    lettercount.push({ 'letter': alphaChar[i], 'number': letter.length })
  }
...
//(7)
const res = letterNumber()
const mostUsedLetter = res.reduce((prev, current) => (prev.number > current.number) ? prev : current)
